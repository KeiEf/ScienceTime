...
class NoteTagView(ListView):
    model = Note
    template_name ='notes/note_all.html'
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if sort == "view":
          tag_posts = Note.objects.filter(tags__slug=self.kwargs.get('tag_slug')).order_by('-views')
        elif sort == "inv_view":
          tag_posts = Note.objects.filter(tags__slug=self.kwargs.get('tag_slug')).order_by('views')
        elif sort == "date":
          tag_posts = Note.objects.filter(tags__slug=self.kwargs.get('tag_slug')).order_by('-post_date')
        elif sort == "inv_date":
          tag_posts = Note.objects.filter(tags__slug=self.kwargs.get('tag_slug')).order_by('post_date')
...
class PostTagView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        sort = self.request.GET.get('sort')
        if sort == "view":
          tag_posts = Post.objects.filter(tags__slug=self.kwargs.get('tag_slug')).order_by('-views')
        elif sort == "inv_view":
          tag_posts = Post.objects.filter(tags__slug=self.kwargs.get('tag_slug')).order_by('views')
        elif sort == "date":
          tag_posts = Post.objects.filter(tags__slug=self.kwargs.get('tag_slug')).order_by('-post_date')
        elif sort == "inv_date":
          tag_posts = Post.objects.filter(tags__slug=self.kwargs.get('tag_slug')).order_by('post_date')
          ...
          tag_posts = Post.objects.filter(
                ...
                | Q(tags__name__icontains=q_word)  | Q(content__icontains=q_word) 
                ).distinct()
...
'''
#=== Ads ===
# View to list all ads
#def ad_list(request):
#    ads = Ad.objects.all()
#    return render(request, 'ads/ad_list.html', {'ads': ads})
'''
# View to display details of a specific ad
...
def ad_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('ad_list')  # Redirect to ad list after saving
    else:
        form = AdForm()
    return render(request, 'ads/ad_form.html', {'form': form})

# View to update an existing ad
